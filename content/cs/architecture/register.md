...
레지스터는 [[cs/architecture/cpu|cpu]]에서 가장 중요한 부품이다. CPU가 처리하는 과정의 모든 중간 값들이 이 곳에 저장되기 때문이다. 레지스터는 프로세서 레지스터라고 쉽게 볼 수 있는데, CPU의 빠른 **데이터 처리를 돕기 위해 사용되는 임시 저장 공간**이라고 생각하면 편하다. ^register-def
...
1. **프로그램 카운터**
프로그램 카운터는 [[cs/architecture/memory|메모리]]에서 다음으로 읽어 들일 [[cs/architecture/readable-data#^address-def|명령어의 주소]]를 저장한다. (**명령어 포인트**라고 부르기도 함). 
</br></br>  ^2017eb
2. **명령어 레지스터**
...
| [[cs/architecture/interrupt#^interrupt-flag-def\|인터럽트 플래그]] | 인터럽트 가능 여부                    |
| 슈퍼바이저 플래그                                                   | 커널 모드로 실행 중인지, 사용자 모드로 실행 중인지 |

5. **스택 포인터**
메모리에서는 실행 중인 프로그램이 **적재**되어 있다. 메모리 내 [[cs/os/process-n-thread#^241a21|스택 영역]]의 최상단 스택 데이터 위치를 가르키는 레지스터가 바로 **스택 포인터**다. 
