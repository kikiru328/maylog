...
**CPU**(Central Processing Unit)는 중앙 처리 부품을 의미한다. CPU는 주요 구성 요소와 빠른 명령어 처리를 위한 기술이 중점적인 부품이다. 크게 CPU 내부는 [[8-ALU|산술논리연산장치(ALU)]]와 제어 장치를 포함한 여러 레지스터들이 있다.
...
1. **프로그램 카운터**
프로그램 카운터는 메모리에서 다음으로 읽어 들일 [[2-readable-data#^address-def|명령어의 주소]]를 저장한다. (**명령어 포인트**라고 부르기도 함). 
</br></br>
2. **명령어 레지스터**
