...
---
<p align="center">
  <img src="https://media1.tenor.com/m/GVk4jB2u_i8AAAAd/coding.gif" alt="note title" width="300">
</p>
...
# Logisitic Regression as a Neural Network

우리가 흔히 알고 있는 Logisitic Regression은 Neural Network와 비슷한 형태를 띄기에 설명이 편리하다.  
입력 데이터에 따라서 분류하는 것은 **이진 분류**(Binary Classification)와 **다중 분류**(Mutliclass Classification)로 나누어진다. 따라서 Logisitic Regression은 Binary Classification 이므로, Neural Network Binary Classification 구조로 설명이 가능하다.  
...
4. ***Vectorization*** 된 `모든 입력 x`를 나타내기 위한 `행렬`은 대문자 X로 표기:   
$$(X \in \mathbb{R}^{n_x \times m})$$
$$\text{X} = \begin{bmatrix} \vert & \vert & & \vert \\ x^{(1)} & x^{(2)} & \dots & x^{(m)} \\ \vert & \vert & & \vert \end{bmatrix}$$
5. output label `y` 또한 `행렬` 구성이 되며, 대문자 Y로 표기.  
...
하지만 Logisitic Regression은 Binary Classification에 효과가 뛰어나지는 않다. 이진 분류는 0과 1로 구분되어야 하지만, Logisitic Regression에 따르면 y의 값이 1보다 클 수도 있으며, -1 이하로 값이 도출 될 수 있기 때문이다.

따라서 이를 해결하기 위해 sigmoid function 이용하게 된다.  

![](https://media.tenor.com/kLek8Kd9GIUAAAAM/neural-network-sigmoid.gif)

sigmoid function은 다음과 같다.  
  
$$\sigma(z) = \frac{1}{1 + e^{-z}},\ \text{if } z \text{ is large, } \sigma(z) \approx \frac{1}{1+0} = 1,\ \text{if } z \text{ is small, } \sigma(z) \approx \frac{1}{1+\infty} = 0$$
...
$\text{Cost Function: } J(w, b) = \frac{1}{m} \sum_{i=1}^{m} \mathscr{L}(\hat{y}^{(i)}, y^{(i)}) = -\frac{1}{m} \sum_{i=1}^{m} \left[ y^{(i)} \log(\hat{y}^{(i)}) + (1 - y^{(i)}) \log(1 - \hat{y}^{(i)}) \right]$
...
![](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2FnscdF%2FbtqFiP6Uc7y%2FLHiNYvzDI5GoAOmX9uCm31%2Fimg.png)
...
Logisitic Regression에서 Gradient Descent를 적용하기 위해 미분 방법이다.  
이전에 Logistic Regression을 통해서 다음과 같은 식을 보았다.  

![](https://img1.daumcdn.net/thumb/R1280x0/?scode=mtistory2&fname=https%3A%2F%2Fblog.kakaocdn.net%2Fdn%2F64rIF%2FbtqFiPszhzk%2FAuvHLYYzapD2d4InYzDxEK%2Fimg.png)
...
[dL / dz = a - y 가 되는 이유](app://obsidian.md/index.html#--%EC%B0%B8%EA%B3%A0-dlay--dz--a---y-%EB%A1%9C-%EB%8F%84%EC%B6%9C%EB%90%98%EB%8A%94-%EC%9D%B4%EC%9C%A0)

마지막 단계에서는 $w$와 $b$에 대한 변화량을 게산해야 한다. $dw_{1}$ 즉 $\frac{dl}{dw_{1}}$ 와 $dw_{2}$, $db$를 구하면 다음과 같다. $dw_{1}\,=\,x_{1}dz\quad dw_{2}\,=\,x_{2}dz\quad db=dz\;$이다. 위 공식들을 사용해서 w와 b는 다음과 같이 업데이트가 된다. 
...
현재는 feature가 2개이기지만, 앞으로 feature 개수가 많아지면 그 만큼 계산을 해주어야 한다. 그리고 계산된 미분값들을 사용해 $w,b$가 업데이트 된다.
